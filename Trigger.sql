...
/**
 * Checks that there could one be one ROOT logger in the table.
 */
CREATE OR REPLACE TRIGGER T2_EFFECTIVE_DOUBLE_ROOT
  BEFORE INSERT ON LOGDATA.CONF_LOGGERS_EFFECTIVE
  REFERENCING NEW AS N
  FOR EACH ROW
 T_EFFECT_TWO_ROOT: BEGIN
  DECLARE LOGGER ANCHOR LOGDATA.CONF_LOGGERS.LOGGER_ID;
