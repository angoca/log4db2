...
CREATE OR REPLACE TRIGGER CHECK_CONF_LOGGER_EFFECTIVE
  ...
  FOR EACH ROW
 T_CHK_CONG_LOGGER: BEGIN ATOMIC
  ...
  ELSEIF (N.LOGGER_ID <> 0) THEN
   ...
   ELSE
    -- Searches the parent ID.
    SET EXISTING_ID = (SELECT LOGGER_ID
      FROM CONF_LOGGERS E
      WHERE E.LOGGER_ID = N.PARENT_ID);
    -- Checks if the parent id is null.
    IF (EXISTING_ID IS NULL) THEN
    SIGNAL SQLSTATE VALUE 'LG003'
      SET MESSAGE_TEXT = 'The given parent does not exist';
    END IF; 
   END IF;
  END IF;
